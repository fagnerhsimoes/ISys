...
using ISys.Infra.CrossCutting.Identity.Extensions;
...
namespace ISys.UI.Web.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> ExternalLoginCallback(string returnUrl = null, string remoteError = null)
        {
            ...
            if (result.Succeeded)
            {
                _logger.LogInformation("Usuário conectado com {Name} provedor.", info.LoginProvider);
                return RedirectToLocal(returnUrl);
            }
            if (result.IsLockedOut)
            {
        ...
        public async Task<IActionResult> ExternalLoginConfirmation(ExternalLoginViewModel model, string returnUrl = null)
        {
            if (ModelState.IsValid)
            {
                ...
                if (info == null)
                {
                    throw new ApplicationException("Erro ao carregar informações de login externo durante a confirmação.");
                }
                var user = new ApplicationUser { UserName = model.Email, Email = model.Email };
                ...
                if (result.Succeeded)
                {
                    ...
                    if (result.Succeeded)
                    {
                        ...
                        // User claim for remove customers data
                        await _userManager.AddClaimAsync(user, new Claim("Room"   , "Remove"));
        ...
        public async Task<IActionResult> ConfirmEmail(string userId, string code)
        {
            ...
            if (user == null)
            {
                throw new ApplicationException($"Não foi possível carregar o usuário de ID '{userId}'.");
            }
        ...
        public IActionResult ResetPassword(string code = null)
        {
            if (code == null)
            {
                throw new ApplicationException("Um código deve ser fornecido para redefinir a senha.");
            }
